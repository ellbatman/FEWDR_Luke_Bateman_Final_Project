...
function loader () {
  ...
  var colors = [
    //blue "rgba(41,171,226,0.4)"
    ["rgba(41,171,226,0.4)", 30, 40, pi/4],
    //aqua "rgba(21,159,148,0.5)"
    ["rgba(21,159,148,0.5)", 15, 10, pi/2],
    //black "rgba(0,0,0,0.5)"
    ["rgba(0,0,0,0.5)",  10, 8,  pi],
    //green "rgba(0,146,69,0.8)"
    ["rgba(0,146,69,0.8)", 10, 4, 1.5*pi],
    //white "rgba(255,255,255,0.7)"
    ["rgba(255,255,255,0.7)", 5, 3,  1.5*pi]
  ];
...
$('.rightNavButton').on('click', function () {
	if ($('.leftMenu').hasClass('activeLeft')) {
			$('.leftMenu').removeClass('activeLeft');
			$('.site').removeClass('slideRight');
			$('.leftNavButton').removeClass('openLeft');
			$('.landingPage h1, .timeline').toggleClass('fade');
			$('#left h1').fadeOut('fast');
	}
	$(this).toggleClass('openRight');
  $('.rightMenu').toggleClass('activeRight');
	$('.site').toggleClass('slideLeft');
	$('.landingPage h1, .timeline').toggleClass('fade');
	$('#right h1').fadeToggle('fast');
});

$('.leftNavButton').on('click', function () {
	if ($('.rightMenu').hasClass('activeRight')) {
			$('.rightMenu').toggleClass('activeRight');
			$('.site').toggleClass('slideLeft');
			$('.rightNavButton').toggleClass('openRight');
			$('.landingPage h1, .timeline').toggleClass('fade');
			$('#right h1').fadeOut('fast');
	}
	$(this).toggleClass('openLeft');
  $('.leftMenu').toggleClass('activeLeft');
	$('.site').toggleClass('slideRight');
	$('.landingPage h1, .timeline').toggleClass('fade');
	$('#left h1').fadeToggle('fast');
});
